...
def run_cpht():
    ...
    if len(dir_name) > 0:
        subprocess.run(
            f'mkdir -p data/{dir_name}', shell=True, stdout=subprocess.DEVNULL)

    # transform jupyter notebook to .py file and move into correct directory
    subprocess.run(
        f'jupyter nbconvert --log-level WARN --to script {file_name}.ipynb', shell=True, stdout=subprocess.DEVNULL)
    if len(dir_name) > 0:
        subprocess.run(f'mv {file_name}.py data/{dir_name}/', shell=True, stdout=subprocess.DEVNULL)
    else:
        subprocess.run(f'mv {file_name}.py data/', shell=True, stdout=subprocess.DEVNULL)

    # sync local directory with remote directory (--delete removes content non-existent in local)
    print(colored('*** Sync started', 'white'))
    subprocess.run(f'rsync -av --delete ./data  {cluster}:~/  &> /dev/null',
    ...
    print(colored('*** Sync started', 'white'))
    subprocess.run(f'rsync -av {cluster}:~/data ./ &> /dev/null',
                   shell=True, stdout=subprocess.DEVNULL)
    print(colored('*** Sync done', 'white'))
    ...
    if open_fig == '':
        try:
            if len(dir_name) > 0:
                subprocess.run(f'open data/fig.png', shell=True, stdout=subprocess.DEVNULL)
            else:
                subprocess.run(f'open data/{dir_name}/fig.png',
                               shell=True, stdout=subprocess.DEVNULL)
        except:
